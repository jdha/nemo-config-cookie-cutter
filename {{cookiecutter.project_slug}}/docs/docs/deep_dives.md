...
!!! abstract "Summary"
    ...
    **This page provide additional details on the parameter choices, initial/surface/boundary conditions and output options of the base {{cookiecutter.project_slug}} simulation.**

---

## Overview :globe_with_meridians:

The {{cookiecutter.project_slug}} integrations have been performed using the NEMO ocean sea-ice model configurations at 1/12$^{\circ}$ nominal horizontal resolution.
...
The {{cookiecutter.project_slug}} configuration has been developed using the Nucleus for European Modelling of the Ocean ([NEMO](https://www.nemo-ocean.eu)) modelling framework version 4.2.2. There have been several modifications to the code
base, which are listed under [Modifications]: #Modifications. Although we are wedded to NEMO 4.2.2 for the CANARI project the repository is also configured to setup {{cookiecutter.project_slug}} at 4.2.3 and 4.2\_HEAD.
...
The ocean model horizontal grid and bathymetry used for the {{cookiecutter.project_slug}} simulations are the same as documented in [Storkey et al (2018)](https://doi.org/10.5194/gmd-11-3187-2018).

The {{cookiecutter.project_slug}} configuration is based on the eORCA12 grid and has a horizontal resolution of 1/12$^{\circ}$. A mask is applied to the eORCA12 grid at run-time providing an *active* domain north of 20$^{\circ}$S 
...
The {{cookiecutter.project_slug}} simulations use the Thermodynamic Equation Of Seawater 2010 (TEOS-10, [Ioc et al., 2010](https://unesdoc.unesco.org/ark:/48223/pf0000188170)) and hence absolute salinity and conservative temperature variables are output rather than practical salinity and potential temperature as in EOS-80.

#### Atmospheric Forcing

The base {{cookiecutter.project_slug}} simluations are forced over the period 1979-2023 by the JRA55-do atmospheric reanalysis ([Tsujino et al, 2018](https://doi.org/10.1016/j.ocemod.2018.07.002)) using the NCAR bulk formulae ([Large and Yeager, 2009](https://doi.org/10.1007/s00382-008-0441-3)).
...
To initialise the {{cookiecutter.project_slug}} integrations, conservative temperature, absolute salinity and ice fields from the [eORCA025 NOC-Near-Present-Day](https://noc-msm.github.io/NOC_Near_Present_Day) (1979) simulations are used.
