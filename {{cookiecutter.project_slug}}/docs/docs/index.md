...
**Welcome to the documentation for the North-Atlatic-ARCtic ({{cookiecutter.project_slug}}) configuration**
...
The {{cookiecutter.project_slug}} configuration is based on NEMO v4.2.2 with a horizontal resolution of 1/12$^{\circ}$.
...
For more details on the {{cookiecutter.project_slug}} configuration see [Deep Dives: Model Configurations].
...
To get started, check out and set up an instance of the {{cookiecutter.project_slug}} GitHub [repository](https://github.com/NOC-MSM/{{cookiecutter.project_slug}}):
...
script has been tested and will checkout, compile and run the {{cookiecutter.project_slug}} (NEMO 4.2.2) code on: ARCHER2 for Cray-MPICH and GNU-MPICH, and Anemone for iFort.

=== "Anemone"
    ```sh
    cd {{cookiecutter.project_slug}}

    ./{{cookiecutter.project_slug}}/scripts/setup/{{cookiecutter.project_slug}}_setup -p $PWD/{{cookiecutter.project_slug}}_RUNS  -r $PWD/{{cookiecutter.project_slug}} -n 4.2.2 -x 2 -m anemone -a impi -c ifort
